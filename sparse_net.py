...
from Resnet_pretrained import resnet50
...
class AttU_Net(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, start_channel=64):
        super(AttU_Net, self).__init__()

        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.Conv1 = conv_block(ch_in=img_ch, ch_out=start_channel)
        self.Conv2 = conv_block(ch_in=start_channel + 64, ch_out=start_channel*2)
        self.Conv3 = conv_block(ch_in=start_channel*2 + 256, ch_out=start_channel*4)
        self.Conv4 = conv_block(ch_in=start_channel*4 + 512, ch_out=start_channel*8)
        self.Conv5 = conv_block(ch_in=start_channel*8 + 1024, ch_out=start_channel*16)
    ...
    def forward(self, x):
        pre_model = resnet50(pretrained=True).cuda().float()
        with torch.no_grad():
            x1t, x2t, x3t, x4t = pre_model(x)
        # encoding path
        x1 = self.Conv1(x)

        x2 = self.Maxpool(x1)
        x2 = torch.cat((x2, x1t), 1)
        x2 = self.Conv2(x2)

        x3 = self.Maxpool(x2)
        x3 = torch.cat((x3, x2t), 1)
        x3 = self.Conv3(x3)

        x4 = self.Maxpool(x3)
        x4 = torch.cat((x4, x3t), 1)
        x4 = self.Conv4(x4)

        x5 = self.Maxpool(x4)
        x5 = torch.cat((x5, x4t), 1)
        x5 = self.Conv5(x5)
