...
model_img = AttU_Net(img_ch=1, output_ch=1, start_channel=start_channel).cuda()
...
for epoch in range(current_epoch, num_epoch):
    ...
    for j in range(0, int(np.floor(len(train_img_index_sum) / batch_size))):
        ...
        ssim_value = pytorch_ssim.ssim(denorm(op_img), denorm(lb_img)).item()
